...
<div class="state-single" ng-controller="AppController">
	<div class="container-fluid" ng-controller="ListController as List" ng-cloak ng-init="List.init('district-list', {'getall': 'admin/districts/{{$state->id}}','search': 'admin/districts/search/{{$state->id}}'})">
					...
					<table ng-if="List.ListService.results.length > 0" ng-cloak class="table table-responsive custom-table" >
						...
						<tbody>
							<tr ng-repeat="district in List.ListService.results">
								...
								<td>[[$index+1]]</td>
								<td>[[district.name]]</td>
								<td><a ng-href="../[[district.id]]/nodal-admins" class="btn btn-primary">View </a>
								</td>
							</tr>
						</tbody>
					</table>
