...
<section class="districtadmin_dash cm-content" ng-controller="AppController" ng-cloak>
	<div class="container-fluid" ng-controller="ListController as List" ng-init="List.cycle ='Admission Cycle';List.year=null">
					...
					<span class="form-group">
						<div class="dropdown">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenu1" id="addmission-cyle-dropdown">
                                ...
                                <li>
                                    <a href="#"
                                        ng-click="List.year={{$year}};List.cycle='{{$year}} - {{$year+1}}'; List.init('student-list', {'getall': 'districtadmin/get/schools/all/{{$district->id}}/?selectedCycle={{$year}}','search': 'districtadmin/search/schools/all/{{$district->id}}/?selectedCycle={{$year}}'})">
                                        {{$year}} - {{$year+1}}
                                    </a>
                                </li>
                                @endforeach
		...
		<div ng-cloak ng-init="List.init('school-list', {'getall': 'districtadmin/get/schools/all/{{$district->id}}','search': 'districtadmin/search/schools/all/{{$district->id}}'})">
			<div class="row">
				<div class="col-sm-6 col-xs-12">
				...
				<div class="col-sm-5 col-xs-12">
					<div class="rt-action  pull-right" ng-controller="DownloadReportController as Download">
						<button  ng-if="List.ListService.results.length > 0" ng-disabled="Download.inProcess" type="button" class="btn btn-warning pull-right" ng-click="Download.triggerDownload('districtadmin/get/schools/all/{{$district->id}}/download?selectedCycle='+List.year)">
				...
				<table class="table table-responsive custom-table">
					...
					<tbody>
						<tr ng-repeat="school in List.ListService.results">
							...
							<td>[[school.name]]</td>
							<td>[[school.udise]]</td>
							<td ng-switch="school.status" class="text-capitalize">
								<span ng-switch-when="ban">Banned</span>
								<span ng-switch-default>[[school.application_status]]</span>
							</td>
						</tr>
					</tbody>
				</table>
