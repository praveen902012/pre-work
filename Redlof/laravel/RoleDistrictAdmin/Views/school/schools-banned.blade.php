...
<section class="districtadmin_dash cm-content" ng-controller="AppController" ng-cloak>
	<div class="container-fluid" ng-controller="ListController as List" ng-init="List.cycle ='Admission Cycle';List.year=null">
							...
							<button class="btn btn-default dropdown-toggle" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
							 ...
							 <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
								@foreach($years as $year)
									<li>
										<a href="#"
								    	ng-click="List.year={{$year}};List.cycle='{{$year}} - {{$year+1}}'; List.init('student-list', {'getall': 'districtadmin/get/schools/banned/{{$district->id}}/?selectedCycle={{$year}}','search': 'districtadmin/search/schools/banned/{{$district->id}}/?selectedCycle={{$year}}'})">
								    		{{$year}} - {{$year+1}}
										</a>
									</li>
								@endforeach
		...
		<div ng-cloak ng-init="List.init('school-list', {'getall': 'districtadmin/get/schools/banned/{{$district->id}}','search': 'districtadmin/search/schools/banned/{{$district->id}}'})">
			<div class="row">
				<div class="col-sm-6 col-xs-12">
				...
				<div class="col-sm-5 col-xs-12">
					<div class="rt-action  pull-right" ng-controller="DownloadReportController as Download">
						<button ng-disabled="Download.inProcess" ng-if="List.ListService.results.length > 0" type="button" class="btn btn-warning pull-right" ng-click="Download.triggerDownload('districtadmin/get/schools/banned/{{$district->id}}/download?selectedCycle='+List.year)">
