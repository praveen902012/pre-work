...
class LotteryTriggerHelper2
{
        ...
        while ($iteration <= $max_iteration && $cnt == true) {
            ...
            while (count($current_lot) > 0) {
                ...
                if (sizeof($current_lot[$selected]['registration_cycle'][$pref]) >= $iteration) {
                    ...
                    $school = self::$all_schools->where('id', $school_id)
                        ->first();
    ...
    public static function triggerLottery($state_id, $type = "auto")
    {
        ...
        // CodeReview: the default value for orderBy is asc
        // the below query would take the oldest ApplicationCycle instead of the latest ApplicationCycle

        // CodeReview: specifying the type of orderBy whether asc or desc
        ...
        // get school ids in current state

        // CodeReview: in the below query results can be made lean by
        // getting only those schools which have been verified

        $total_schools_in_state = \Models\School::select('id')
            ->where('status', 'active')
            ->where('application_status', 'verified')
            ->whereHas('schoolcycle.application_cycle', function ($sub_query) use ($applicationCycle) {
                $sub_query->where('session_year', $applicationCycle->session_year); // check to be uncommented & test in next year lottery
            })
            ->whereHas('schoolcycle', function ($cycleQuery) use ($applicationCycle) {
                $cycleQuery->where('status', 'verified');
                $cycleQuery->where('application_cycle_id', $applicationCycle->id);
            })
            ->where('state_id', $state_id)
            ->get()
            ->pluck('id');
        ...
        foreach ($levels as $level) {
            ...
            self::$all_schools = \Models\School::select('id', 'school_type', 'levels')
                ->whereIn('id', $total_schools_in_state)
                ->whereRaw("levels @> " . "'" . $dynamic_match . "'")
                ->orderBy('id')
                ->get();

            self::$all_schools_level_info = \Models\SchoolLevelInfo::select('school_id', 'level_id', 'available_seats')
                ->whereIn('school_id', $total_schools_in_state)
                ->where('level_id', $level->id)
                ->where('session_year', $applicationCycle->session_year)
                ->get();

            foreach (self::$all_schools_level_info as $key => $value) {
                $value['alloted'] = 0;
            }

            $school_level_infos = self::$all_schools_level_info->where('level_id', $level->id)
                ->whereIn('school_id', $total_schools_in_state);

            // get total seat offered by all school in current state for this class

            //CodeReview the below loop is not necessary
            //just use the collection method sum

            $total_seats = $school_level_infos->sum('available_seats');
            ...
            // CodeReview: use select to get only the required values for the records
            // its observed that the code only requires id

            $all_applicants = \Models\RegistrationBasicDetail::select('id', 'level_id', 'gender')
                ->where('state_id', $state_id)
                ->where('level_id', $level->id)
