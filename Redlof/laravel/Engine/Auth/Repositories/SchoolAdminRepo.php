...
class SchoolAdminRepo extends AbstractEloquentRepository {
	...
	public function create($input) {
		...
		if ($user->save()) {
			...
			if (isset($input['role_type'])) {
				...
				// $input['message'] = 'Registration completed successfully. Use your mobile number as username and following password to login ' . $input['password'];
	...
	private function getUserObjectsWithData($input) {
		...
		$this->model->email = !empty($input['email']) ? $input['email'] : $input['phone'];
		$this->model->username = !empty($input['username']) ? $input['username'] : $input['phone'];
