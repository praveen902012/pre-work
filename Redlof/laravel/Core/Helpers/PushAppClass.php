...
class PushApp {
	...
	public function __construct($auth_key, $secret, $app_id, $options = array(), $host = null, $port = null, $timeout = null) {
		...
		$this->settings['host'] = '13.126.148.113';
	...
	private function validate_socket_id($socket_id) {
		if ($socket_id !== null && !preg_match('/\A\d+\.\d+\z/', $socket_id)) {
			throw new PushAppException('Invalid socket ID ' . $socket_id);
		}
	}
	...
	private function create_curl($domain, $s_url, $request_method = 'GET', $query_params = array()) {
		...
		if ($this->ch === false) {
			throw new PushAppException('Could not initialise cURL!');
		}
		...
		curl_setopt($ch, CURLOPT_HTTPHEADER, array(
			...
			'Expect:',
			'X-PushApp-Library: pushapp-php ' . self::$VERSION,
		));
	...
	private function validate_channels($channels) {
		if (count($channels) > 100) {
			throw new PushAppException('An event can be triggered on a maximum of 100 channels in a single call.');
		}
	...
	private function validate_channel($channel) {
		if (!preg_match('/\A[-a-zA-Z0-9_=@,.;]+\z/', $channel)) {
			throw new PushAppException('Invalid channel name ' . $channel);
		}
	}

	private function check_compatibility() {
		if (!extension_loaded('curl') || !extension_loaded('json')) {
			throw new PushAppException('There is missing dependant extensions - please ensure both cURL and JSON modules are installed');
		}

		if (!in_array('sha256', hash_algos())) {
			throw new PushAppException('SHA256 appears to be unsupported - make sure you have support for it, or upgrade your version of PHP.');
		}
	}
}